...
double focalLenght,baseLine = 6,depth,disp;
...
static void on_trackbar( int, void* )
{
   ...
   disp = abs(c[0] - d[0]);
   depth = baseLine*focalLenght/disp;
   fprintf(stderr,"l[%d] h[%d] x_posL[%i] \n",low,high,c[0]);
   fprintf(stderr,"l[%d] h[%d] x_posR[%i] \n",low,high,d[0]);
   fprintf(stderr,"disp[%f] FL[%f] Depth[%f]\n",disp,focalLenght,depth);
...
int main()
{   
   focalLenght = sqrt(pow(833.349949,2) + pow(840.921656,2));
   ...
   // {
      ...
      // camera0 >> 
      frame0 = imread("Data_Depth_Jarak/kuis/frameL_94.jpg",1);
      // camera1 >> 
      frame1 = imread("Data_Depth_Jarak/kuis/frameR_94.jpg",1);

      imshow("gambar kiri", frame0);
      imshow("gambar kanan", frame1);
